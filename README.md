# Mon Profil :

**Futur élève en 1re année de cycle ingénieur à l'ISEN Brest**

*Actuellement en 2e année de classe préparatoire intégrée - Biologie et Santé à l'ISEN Brest*

**Je suis à la recherche d'une alternance d'une durée de 3 ans et commençant en septembre 2026.**

### Me contacter
- [LinkedIn](https://www.linkedin.com/in/mathys-u-1747763a0/)
- [Mail](ulvoasmathys@gmail.com)

### Compétences Techniques informatiques :
- **Langage :** Python (bases : structures de données, gestion de fichiers, matplotlib)
- **Outils :** VS Code, GitHub, ImageJ, Microsoft Excel, Word, Powerpoint

### Compétences techniques diverses :
- Gestion de projets
- Sciences étudiées : Anatomie, Physiologie, Génétique, Biologie cellulaire, Microbiologie, Histologie, Pharmaco-toxicologie et Biochimies (structurale, métabolique)

### Projets effectués
- **Jeu du labyrinthe en ligne de commande :** Jeu d'aventure textuel en Python avec gestion de fichiers et logique métier.
- **Simulateur physique simple interactif :** interface graphique (matplotlib), gravité et collisions

### Projets envisagés :
- Création de mon réveil-lumière développé par "Mon Electro Box"
- Découverte du Starter Kit Arduino R4

---
*“Passionné par l'intersection entre l'ingénierie et la médecine.”*
